...
export class AuthenticationService {
  ...
  register(username: string, password: string, confirmPassword: string, name: string, phone: string) {
    return this.httpClient.post<any>(API_URL + '/register', {username, password, confirmPassword, name, phone})
      .pipe(map(user => {localStorage.setItem('currentUser', JSON.stringify(user));
        this.currentUserSubject.next(user);
        return user;
      }));
